...
def matrix_norm(mixed_state, pure_state):
    ...
    return np.sum(
        np.abs(mixed_state - np.outer(pure_state, np.conj(pure_state))))
...
def compare_circuits(num_wires, params):
    ...
    # QHACK #
    # define devices
    dev = qml.device("default.qubit", wires=int(num_wires))
    dev2 = qml.device("default.qubit", wires=int(num_wires))
