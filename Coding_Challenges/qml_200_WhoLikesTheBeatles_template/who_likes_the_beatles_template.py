...
def distance(A, B):
    ...
    dev = qml.device("default.qubit", wires=qubit_number * 2 + 1)
    ...
    return np.sqrt(2.0 * (1.0 - np.sqrt(2.0 * circuit()[0] - 1.0)))
...
def predict(dataset, new, k):
    ...
    return (
        "YES"
        if len([i for i in output if i == "YES"]) > len(output) / 2 else "NO",
