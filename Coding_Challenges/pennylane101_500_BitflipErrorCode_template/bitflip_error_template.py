...
def error_wire(circuit_output):
    ...
    prob = circuit_output
    err_readout = np.zeros([4])
    err_readout[0] = prob[0] + prob[4]
    err_readout[1] = prob[3] + prob[7]
    err_readout[2] = prob[2] + prob[6]
    err_readout[3] = prob[1] + prob[5]
    return err_readout
...
def density_matrix(alpha):
    ...
    psi = alpha * np.array([1, 0], dtype=float) + np.sqrt(
        1 - alpha**2) * np.array([0, 1], dtype=float)
