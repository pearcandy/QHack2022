...
def classify_ising_data(ising_configs, labels):
    ...
    def layer(W):
        qml.Rot(0, W[0, 0], W[0, 1], wires=0)
        qml.Rot(0, W[1, 0], W[1, 1], wires=1)
        qml.Rot(0, W[2, 0], W[2, 1], wires=2)
        qml.Rot(0, W[3, 0], W[3, 1], wires=3)
    ...
    num_qubits = 4
    num_layers = 3
    weights_init = 0.01 * np.random.randn(
        num_layers, num_qubits, 2, requires_grad=True)
    bias_init = np.array(0.0, requires_grad=True)

    #optimizer
    opt = optimize.RMSPropOptimizer(0.15)
    batch_size = 15
    ...
    for it in range(15):
        ...
        # Compute accuracy
        predictions = [
            int(np.sign(variational_classifier(weights, bias, x))) for x in X
        ]
...
if __name__ == "__main__":
    inputs = np.array(sys.stdin.read().split(","),
                      dtype=int,
                      requires_grad=False).reshape(DATA_SIZE, -1)
