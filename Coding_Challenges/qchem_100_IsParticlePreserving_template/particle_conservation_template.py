...
def is_particle_preserving(circuit, n):
    ...
    # QHACK #
    state = basis_states(n)

    nums = []  # original particle num
    for i in range(len(state)):
        nums.append(np.sum(state[i]))

    nums2 = []  # particle num after performing circuit
    for i in range(len(state)):
        output = circuit(state[i])
        num2 = 0
        for j in range(len(output)):
            num2 = num2 + np.abs(output[j])**2 * nums[j]
        nums2.append(num2.numpy())
